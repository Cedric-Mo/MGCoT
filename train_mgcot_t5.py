...
from model_t5.modeling_mgcot_t5 import T5ForConditionalGeneration
from transformers.models.t5.tokenization_t5_fast import T5TokenizerFast
from model_t5.trainer_seq2seq_custom import CustomSeq2SeqTrainer
from model_t5.training_args_seq2seq_custom import CustomSeq2SeqTrainingArguments
...
def main():
    ...
    def preprocess_function(examples):
        ...
        # if "rationale" in examples.keys() and examples["rationale"]:
        #     rationales = examples["rationale"]
        #     inputs_rationales = []
        #     for inp, rat in zip(inputs, rationales):
        #         if rat is not None:
        #             inputs_rationales.append(inp + "\n" + rat)
        #         else:
        #             inputs_rationales.append(inp)
        #     inputs = inputs_rationales
...
#         model_inputs["col_ids"] = token_col_ids
    ...
    def postprocess_text(preds, labels, metric_name):
        ...
        if metric_name == "metric/sacrebleu.py":
            labels = [[label] for label in labels]
        elif metric_name == "metric/rouge.py":
            import nltk
    ...
    def compute_metrics(eval_preds):
        ...
        # Metric
        metric_names = ["metric/sacrebleu.py", "metric/rouge.py"]
#         metric = load_metric(metric_name)
        ...
        # Some simple post-processing
            predictions, references = postprocess_text(decoded_preds, decoded_labels, metric_name)
            ...
            if metric_name == "metric/sacrebleu.py":
                ...
                result[f"rouge-1"] = res[f"rouge1"].mid.fmeasure
                result[f"rouge-2"] = res[f"rouge2"].mid.fmeasure
                result[f"rouge-L"] = res[f"rougeL"].mid.fmeasure
    ...
    if training_args.do_eval:
        ...
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                eval_preds = [pred.strip() if isinstance(pred, str) else six.ensure_text(pred, "utf-8", "ignore").strip() for pred in eval_preds]
                output_eval_preds_file = os.path.join(training_args.output_dir, "val_preds_seq2seq.txt")
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                test_preds = [pred.strip() if isinstance(pred, str) else six.ensure_text(pred, "utf-8", "ignore").strip() for pred in test_preds]
